...
class Paths
{
	...
	public static function listOggFilesInSongs():Array<String> {
        var assetsSongs = [for (path in mobile.backend.AssetUtils.listAssetsFromPrefix("assets/songs")) if (path.endsWith(".ogg")) path];
        var contentSongs = [for (path in mobile.backend.AssetUtils.listAssetsFromPrefix("content/songs")) if (path.endsWith(".ogg")) path];
